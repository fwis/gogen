...
func keybindings(g *gocui.Gui) error {
	if err := g.SetKeybinding("side", gocui.KeyCtrl2, gocui.ModNone, nextView); err != nil {
		return err
	}
	if err := g.SetKeybinding("main", gocui.KeyCtrl2, gocui.ModNone, nextView); err != nil {
		return err
	}
...
func layout(g *gocui.Gui) error {
	maxX, maxY := g.Size()
	if v, err := g.SetView("side", 80, -1, maxX, maxY-4); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
		v.Highlight = true
	...
	if v, err := g.SetView("main", -1, -1, 80, maxY-4); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
		/*
	...
	if v, err := g.SetView("cmd", -1, maxY-4, maxX, maxY); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
		/*
...
func commandEditor(v *gocui.View, key gocui.Key, ch rune, mod gocui.Modifier) {
	...
	case key == gocui.KeyTab:
		...
		v.Write([]byte("AAA"))
		v.MoveCursor(3, 0, true)
		//v.MoveCursor(4, 0, false)
		//v.EditNewLine()
		//v.MoveCursor(0, 1, false)
		//v.Write([]byte(oldCmd))
